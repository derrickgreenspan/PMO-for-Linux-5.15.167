...
/* Derrick Greenspan added this *
 * Get the struct resource, so we can then get the starting and ending 
 * addresses. 
 * 
 * We will expect that know the name of the device as described in /dev/dax
 * be specified.
 */
struct resource *get_dax_dev_resource(char *dev_name)
{
        ...
        device = bus_find_device_by_name(&dax_bus_type, NULL, dev_name);

        if(device == NULL)
                goto err;

        dev_dax = to_dev_dax(device);

        if(dev_dax == NULL)
                goto err;

        region = dev_dax->region;
        printk(KERN_INFO "dev_pagemap start:%x end:%x\n", dev_dax->pgmap->altmap.base_pfn,
                        dev_dax->pgmap->altmap.end_pfn);
